...
import { reportAsOfLabel, reportPeriodLabel } from "@/lib/reports-data";
  ...
  head: () => ({
    meta: [
      { title: "Financial Report — Financial Reports — Fortress Reserve" },
      {
        name: "description",
        content:
          "Bundled financial report: income statement, balance sheet, cash flow, and equity statement with notes.",
      },
      { property: "og:title", content: "Financial Report — Fortress Reserve" },
      { property: "og:description", content: "Combined print-friendly financial statements." },
  ...
  return (
    <section className="break-inside-avoid border-b border-dashed border-reserve-navy/15 py-8 first:pt-0 last:border-0 dark:border-white/15 print:break-after-page print:border-0">
      <div className="mb-5 text-center">
        <p className="text-[10px] font-semibold tracking-[0.18em] text-reserve-slate uppercase">
          {entityName}
        </p>
...
function FinancialReportPage() {
  const { entries, entityName } = useReportsStore();
  const periodLabel = reportPeriodLabel(entries);
  const asOfLabel = reportAsOfLabel();
  return (
    <div className="rounded-2xl border border-reserve-navy/10 bg-white p-5 shadow-sm sm:p-10 dark:border-white/10 dark:bg-reserve-navy/40 print:border-0 print:shadow-none">
      ...
      <div className="mb-8 border-b-2 border-reserve-navy pb-6 text-center dark:border-white">
        <p className="text-[11px] font-semibold tracking-[0.2em] text-reserve-slate uppercase">
          {entityName}
        </p>
        <h2 className="mt-2 text-2xl font-bold tracking-tight text-reserve-navy sm:text-3xl dark:text-white">
          Financial Report
        </h2>
        <p className="mt-1 text-sm text-reserve-slate">{periodLabel}</p>
        <button
          type="button"
      ...
      <DocPage title="Income Statement" subtitle={periodLabel} entityName={entityName}>
        <IncomeStatementBody />
      </DocPage>

      <DocPage title="Balance Sheet" subtitle={asOfLabel} entityName={entityName}>
        <BalanceSheetBody />
      </DocPage>

      <DocPage title="Statement of Cash Flows" subtitle={periodLabel} entityName={entityName}>
        <CashFlowBody />
      </DocPage>
      ...
      <section className="pt-8 print:break-after-page">
        <div className="mb-5 text-center">
          ...
          <h3 className="mt-1 text-lg font-bold text-reserve-navy dark:text-white">
            Notes to the Financial Statements
          </h3>
          <p className="text-xs text-reserve-slate">{periodLabel}</p>
        </div>
        <ol className="list-decimal space-y-3 pl-5 text-[13px] leading-relaxed text-reserve-navy/90 dark:text-white/85">
          <li>
            <span className="font-semibold">Basis of preparation.</span> These statements are
            prepared on the accrual basis. Revenue is recognized when earned and expenses when
            incurred, regardless of when cash changes hands.
          </li>
          <li>
            <span className="font-semibold">Source of figures.</span> Every balance shown is derived
            entirely from the manual journal entries recorded in Financial Reports. No depreciation,
            accruals, or other automated postings are applied unless entered as a journal entry.
          </li>
        </ol>
      </section>

      <p className="mt-10 border-t border-reserve-navy/10 pt-3 text-center text-[11px] text-reserve-slate dark:border-white/10">
        {entityName} — manually recorded books, not connected to wallet or reserve activity.
      </p>
    </div>
  );
}
