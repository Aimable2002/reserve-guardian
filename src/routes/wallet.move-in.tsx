...
export const Route = createFileRoute("/wallet/move-in")({
  head: () => ({
    meta: [
      { title: "Move from Reserve · Wallet" },
      { name: "description", content: "Pull funds from a reserve back into your wallet." },
      { property: "og:title", content: "Move from Reserve · Wallet" },
      { property: "og:description", content: "Pull funds from a reserve back into your wallet." },
...
function MoveInPage() {
  ...
  const [reserveId, setReserveId] = useState<string>(
    store.reserves.find((r) => r.current > 0)?.id ?? store.reserves[0]?.id ?? "",
  );
  ...
  const valid = !!reserve && Number.isFinite(value) && value > 0 && value <= (reserve?.current ?? 0);

  const submit = () => {
    if (!reserve) return;
    const ok = store.reserveToWallet(reserve.id, value);
    if (!ok) return toast.error("Reserve doesn't have enough to move.");
    toast.success(`Moved ${formatMoney(value)} → Wallet`);
    navigate({ to: "/wallet" });
  };
      ...
      >
        <header className="flex items-center justify-between py-6">
          ...
          <span className="inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-widest text-reserve-slate">
            <ArrowDownToLine className="size-3" /> Move from Reserve
          </span>
        </header>
        ...
        {step === "form" ? (
          <section className="mt-6 space-y-4 rounded-2xl border border-reserve-navy/5 bg-white p-5 shadow-sm">
            <div>
              <Label className="text-[11px] uppercase tracking-wider text-reserve-slate">Source Reserve</Label>
              <select
                value={reserveId}
              ...
              >
                {store.reserves.map((r) => (
                  <option key={r.id} value={r.id} disabled={r.current <= 0}>
            ...
            <div>
              ...
              <p className="mt-1 text-[10px] text-reserve-slate">
                Available: {reserve ? formatMoney(reserve.current) : "—"}
              </p>
            </div>
            <Button
              disabled={!valid}
          ...
          <section className="mt-6 rounded-2xl border border-reserve-navy/5 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-reserve-slate">{reserve?.name} → Wallet</p>
