...
import { reportAsOfLabel, trialBalanceRows } from "@/lib/reports-data";
...
export const Route = createFileRoute("/reports/trial-balance")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Trial balance proving total debits equal total credits." },
      { property: "og:title", content: "Trial Balance — Fortress Reserve" },
      {
        property: "og:description",
        content: "Trial balance with debit and credit columns and balancing totals.",
      },
...
function TrialBalancePage() {
  ...
  return (
    <ReportShell title="Trial Balance" subtitle={reportAsOfLabel()}>
        ...
        <table className="w-full min-w-[480px] text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.code} className="border-b border-reserve-navy/5 dark:border-white/5">
                ...
                <td className="px-3 py-2 text-right">
                  {r.debit !== 0 && (
                    <Money value={r.debit} className="text-reserve-navy dark:text-white/90" />
                  )}
                </td>
                <td className="px-2 py-2 text-right">
                  {r.credit !== 0 && (
                    <Money value={r.credit} className="text-reserve-navy dark:text-white/90" />
                  )}
                </td>
              </tr>
            ))}
            <tr className="border-t-2 border-reserve-navy font-bold dark:border-white">
              <td
                className="px-2 py-2.5 text-xs tracking-[0.14em] text-reserve-navy uppercase dark:text-white"
                colSpan={2}
              >
                Totals
              </td>
